  ...
  return (
    <div className="relative mb-3 h-60 w-full overflow-hidden rounded-2xl border border-white/15">
      <motion.div whileHover={{ scale: 1.05 }} transition={{ duration: 0.4 }} className="relative h-full w-full">
        <Image src={src} alt={alt} fill sizes="(max-width: 500px) 100vw, 50vw" className="object-cover" />
      </motion.div>
      <div className="pointer-events-none absolute left-1/2 inset-0 bg-linear-to-t from-black/5 via-transparent to-white/10" />
    </div>
  );
}
